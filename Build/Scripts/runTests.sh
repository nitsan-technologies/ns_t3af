...
loadHelp() {
    ...
    read -r -d '' HELP <<EOF
TYPO3 core–style test runner for ns_t3af. Execute unit, functional and other test suites in
...
Options:
    ...
    -t <12|13|14>
        ...
        Specifies the TYPO3 major version used for Composer constraints
            - 12: use TYPO3 v12 (composer pins in this script)
...
case ${TEST_SUITE} in
    cgl)
        ...
        COMMAND="php -dxdebug.mode=off .Build/bin/php-cs-fixer fix -v ${DRY_RUN_OPTIONS} --config=.php-cs-fixer.php --using-cache=no"
        ...
        ${CONTAINER_BIN} run "${CONTAINER_COMMON_PARAMS[@]}" --name composer-install-highest-${SUFFIX} -e COMPOSER_CACHE_DIR=.cache/composer -e COMPOSER_ROOT_VERSION=${COMPOSER_ROOT_VERSION} ${IMAGE_PHP} /bin/bash -c "
            if [ ${TYPO3_VERSION} -eq 12 ]; then
              composer require --no-ansi --no-interaction --no-progress --no-install \
                ...
                typo3/cms-frontend:^12.4 || exit 1
            fi
            if [ ${TYPO3_VERSION} -eq 13 ]; then
              composer require --no-ansi --no-interaction --no-progress --no-install \
                ...
                typo3/cms-frontend:^13.4 || exit 1
            fi
            if [ ${TYPO3_VERSION} -eq 14 ]; then
              composer require --no-ansi --no-interaction --no-progress --no-install \
                ...
                typo3/cms-frontend:^14.3 || exit 1
            fi
    ...
    functional)
        ...
        case ${DBMS} in
            ...
            postgres)
                ...
                ;;
            sqlite)
                # tmpfs for sqlite DB files (same layout as typo3/testing-framework functional tests)
                SQLITE_DBS_DIR="${ROOT_DIR}/.Build/public/typo3temp/var/tests/functional-sqlite-dbs"
                mkdir -p "${SQLITE_DBS_DIR}"
                ${CONTAINER_BIN} run "${CONTAINER_COMMON_PARAMS[@]}" --name functional-${SUFFIX} ${XDEBUG_MODE} -e XDEBUG_CONFIG="${XDEBUG_CONFIG}" -e typo3DatabaseDriver=pdo_sqlite --tmpfs "${SQLITE_DBS_DIR}:rw,noexec,nosuid" ${IMAGE_PHP} "${COMMAND[@]}"
                SUITE_EXIT_CODE=$?
                ;;
        esac
        ;;
    lint)
        ...
        ;;
    rector)
        if [[ ! -f Build/rector/rector.php ]]; then
            echo "rector: Build/rector/rector.php not found, skipping." >&2
            SUITE_EXIT_CODE=0
        else
        DRY_RUN_OPTIONS=''
        if [ "${DRY_RUN}" -eq 1 ]; then
        ...
        ;;
    fractor)
        if [[ ! -f Build/fractor/fractor.php ]]; then
            echo "fractor: Build/fractor/fractor.php not found, skipping." >&2
            SUITE_EXIT_CODE=0
        else
        DRY_RUN_OPTIONS=''
        if [ "${DRY_RUN}" -eq 1 ]; then
