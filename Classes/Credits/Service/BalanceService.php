...
final class BalanceService
{
    ...
    public function fetch(): array
    {
        ...
        $cached = $this->responseCache->get(CreditsApiResponseCache::SCOPE_BALANCE, $domain, $token);
        if ($cached !== null) {
            return $cached;
        }

        $payload = $this->apiClient->balance($domain, $token);
        if (($payload['not_modified'] ?? false) === true) {
            return $payload;
        }

        $this->responseCache->set(
            CreditsApiResponseCache::SCOPE_BALANCE,
            $domain,
            $token,
            $payload,
            CreditsApiResponseCache::TTL_BALANCE,
        );
        $this->runtimeSettings->touchBalanceSynced();
