 ...
 * @internal
 */
final class CurrentPlanService
{
    ...
    public function fetch(): array
    {
        ...
        $cached = $this->responseCache->get(CreditsApiResponseCache::SCOPE_CURRENT_PLAN, $domain, $token);
        if ($cached !== null) {
            return $cached;
        }

        $payload = $this->apiClient->currentPlan($domain, $token);
        if (($payload['not_modified'] ?? false) === true) {
            return $payload;
        }

        $this->responseCache->set(
            CreditsApiResponseCache::SCOPE_CURRENT_PLAN,
            $domain,
            $token,
            $payload,
            CreditsApiResponseCache::TTL_CURRENT_PLAN,
        );
