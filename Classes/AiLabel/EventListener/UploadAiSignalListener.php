...
/**
 * Suggest AI involvement when an uploaded file already carries an AI DigitalSourceType.
 */
final class UploadAiSignalListener
{
    public function __construct(
        private readonly IptcDigitalSourceTypeService $iptcDigitalSourceTypeService,
        private readonly AiLabelRecorderInterface $recorder,
    ) {}
    ...
    public function __invoke(AfterFileAddedEvent $event): void
    {
        ...
        $value = $this->iptcDigitalSourceTypeService->read($file);
        if (!$this->iptcDigitalSourceTypeService->isAiGeneratedSource($value)) {
            return;
        }
